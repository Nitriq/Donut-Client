...
import static net.minecraft.realms.RealmsMth.*;
...
public class BetterChat extends GuiNewChat {
    ...
    public BetterChat(Minecraft mcIn) {
        super(mcIn);
        this.mc = mcIn;
    }
    ...
    public void drawChat(int updateCounter) {
        ...
        percent = Animation.clamp(percent, 0, 1);
        if (this.mc.gameSettings.chatVisibility != EntityPlayer.EnumChatVisibility.HIDDEN) {
            ...
            if (j > 0) {
                ...
                int k = ceil((float) this.getChatWidth() / f1);
                ...
                for (int i1 = 0; i1 + this.scrollPos < this.drawnChatLines.size() && i1 < i; ++i1) {
                    ...
                    if (chatline != null) {
                        ...
                        if (j1 < 200 || flag) {
                            ...
                            d0 = 1.0D - d0;
                            d0 = d0 * 10.0D;
                            d0 = Animation.clamp((float) d0, (float) 0.0D,(float) 1.0D);
                            d0 = d0 * d0;
    ...
    private void setChatLine(IChatComponent chatComponent, int chatLineId, int updateCounter, boolean displayOnly) {
        ...
        int i = floor((float) this.getChatWidth() / this.getChatScale());
    ...
    public IChatComponent getChatComponent(int mouseX, int mouseY) {
        if (!this.getChatOpen()) {
            ...
            if (j >= 0 && k >= 0) {
                ...
                if (j <= floor((float) this.getChatWidth() / this.getChatScale()) && k < this.mc.fontRendererObj.FONT_HEIGHT * l + l) {
    ...
    public static int calculateChatboxWidth(float scale) {
        int i = 320;
        int j = 40;
        return floor(scale * 280.0F + 40.0F);
    }

    public static int calculateChatboxHeight(float scale) {
        int i = 180;
        int j = 20;
        return floor(scale * 160.0F + 20.0F);
    }
